...
/* 预定义read/write到符号表 */
static void predefine_read_write_semantic(SymbolTable* table) {
    if (!find_symbol(table, "read")) {
        Symbol* read_sym = new_function_symbol("read", new_type_basic(TYPE_INT), NULL, 0);
        insert_symbol(table, read_sym);
    }
    if (!find_symbol(table, "write")) {
        ParamList* param = new_param_list("x", new_type_basic(TYPE_INT));
        Symbol* write_sym = new_function_symbol("write", new_type_basic(TYPE_INT), param, 0);
        insert_symbol(table, write_sym);
    }
}

/* 执行语义分析（返回上下文，调用者释放） */
SemanticContext* semantic_analysis_with_context(Node* root) {
    if (root == NULL) return NULL;
    
    g_context = init_semantic_context();
    
    /* 预定义read/write函数，使语义分析不会报未定义错误 */
    predefine_read_write_semantic(g_context->global_table);
    
    analyze_program(g_context, root);
    SemanticContext* result = g_context;
    g_context = NULL;
    return result;
}
...
void analyze_stmt(SemanticContext* context, Node* node) {
    ...
    // 处理CompSt（语句块）: CompSt
    if (num_children == 1 && is_node_name(first_child, "CompSt")) {
        analyze_comp_st(context, first_child, context->current_return_type);
        return;
    }
    ...
    // 处理WHILE语句: WHILE LP Exp RP Stmt
    else if (num_children == 5 && is_node_name(first_child, "WHILE")) {
        Node* exp_node = get_child(node, 2);
        if (exp_node != NULL && is_node_name(exp_node, "Exp")) {
            ExpTypeInfo exp_info = analyze_exp(context, exp_node);
            if (exp_info.type != NULL) {
        ...
        Node* body = get_child(node, 4);
        if (body != NULL) {
            analyze_stmt(context, body);
        }
    }
}
...
ExpTypeInfo analyze_exp(SemanticContext* context, Node* node) {
    ...
    // 处理一元负号：MINUS Exp
    else if (num_children == 2 && is_node_name(first_child, "MINUS")) {
        ExpTypeInfo inner = analyze_exp(context, get_child(node, 1));
        if (inner.type != NULL) {
            info.type = copy_type(inner.type);
        } else {
    ...
    // 处理一元NOT：NOT Exp
    else if (num_children == 2 && is_node_name(first_child, "NOT")) {
        ExpTypeInfo inner = analyze_exp(context, get_child(node, 1));
        if (inner.type != NULL) {
            info.type = copy_type(inner.type);
        } else {
    ...
    // 处理括号表达式：LP Exp RP
    else if (num_children == 3 && is_node_name(first_child, "LP") && is_node_name(get_child(node, 2), "RP")) {
        ExpTypeInfo inner = analyze_exp(context, get_child(node, 1));
        info.type = inner.type ? copy_type(inner.type) : NULL;
        info.is_lvalue = inner.is_lvalue;
    }
    // 处理ID节点
